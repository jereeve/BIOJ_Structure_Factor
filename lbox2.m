...
if strcmp(get(handles.figure1,'SelectionType'),'open')
    ...
    if  handles.is_dir(handles.sorted_index(index_selected))
        ...
        switch ext
            ...
            otherwise
                try
                    global dimx dimy cdata v;
                    cdata = uint16(imread(filename,'tif'));
                    imfh = imageprocessing;
                    imdata = guidata(imfh);
                    [dimx dimy dimz] = size(cdata);
                    if max(cdata) > 32768 % This is a fix to take a large number off of signed 64 bit tiffs
                        cdata = double(cdata) - (ones(dimx,dimy)*32768);
                    end
                    if dimz ~= 1 %This is a fix so that we can import edited images.
                        newdata = cdata(:,:,1) + cdata(:,:,2) + cdata(:,:,3);
                        cdata = double(newdata);
                    else
                        cdata = double(cdata);
                    end    
                    axes(imdata.circplot);
                    imagesc(cdata);
                    axes(imdata.surfplot);
                    surfc(cdata,'edgecolor','none');
                    axis([-0 dimx -0 dimy 0 500], 'auto z');
                    v = axis;
                    colormap jet;
                    set(imdata.surfplot, 'buttondownfcn', @surfmax)
                    delete(lbox2);
                catch ex
                    errordlg(...
