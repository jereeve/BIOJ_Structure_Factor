...
function output = onepffun(var,data)
A(1) = var(1);
A(2) = var(2);
A(3) = var(3);
A(4) = var(4);
...
% 1PF
output = A(1) .* (3 .* (cos(the + A(2)).^2 - (1/3)) .* A(3) - cos(the + A(2)).^2 + 1) .* exp(-(rh - (A(4)./(1 + A(5).*cos(the)))).^2/(2 .* A(6))) + A(7) .* erfc((rh - (A(4)./(1 + A(5).*cos(the)))) ./ A(8)) + A(9);
